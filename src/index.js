...
class Bree extends EventEmitter {
  ...
  async init() {
    debug('init');
    debug('mysqlDatabase', this.config.mysqlDatabase);
    ...
    if (this.config.mysqlDatabase) {
      ...
      setInterval(async () => {
        await getLeader(connection);
        monitorServer(connection);
        getRecurringJobsData(connection);
      }, 1000);
      await service.getJobsData(
        connection,
        this.config.mysqlDatabase.maxJobs,
        this.config.jobs
      );
      const countOfRunningJobs = 0;
      setInterval(async () => {
        await service.updateFailedJobs(connection, this.config.jobs, this.config.mysqlDatabase.retryCount);
        await service
          .getJobsData(connection, this.config.mysqlDatabase.maxJobs)
          .then(async (jobs) => {
            ...
            addedJobsToBree.forEach((job) => {
              if (countOfRunningJobs <= this.config.mysqlDatabase.maxJobs) {
                this.start(job.name);
              } else {
      ...
      }, this.config.mysqlDatabase.jobInterval);
    }
    // Validate root
